...
class CommandPush extends Command {
    async execute(device){
        ...
        if(result.success){
            ...
            alert("Couldn't send push: " + result.errorMessage);
        };
        return result;		
    };
    //abstract
...
class CommandSay extends CommandPush{
    ...
    customizePush({device,push}){
        push.say = prompt("What do you want to say out load on the device?");;
        return push;
    }
}
class CommandOpenApp extends CommandPush{
    ...
    customizePush({device,push}){
        push.app = prompt("What app do you want to open?");;
        return push;
    }
}
...
class CommandWrite extends CommandPush{
    ...
    customizePush({device,push}){
        push.clipboard = prompt(`What do you want to write on your ${device.deviceName}?`);
        if(!push.clipboard) return;
...
class CommandRenameDevice extends Command{
	...
	async execute(device){
        var deviceName = window.prompt(`What do you want to name ${device.deviceName}?`,device.deviceName);
...
class CommandTestLocalNetwork extends Command{
     ...
     */
	async execute(device){
        this.showToast({text:`Testing local network on ${device.deviceName}`});
        try{
