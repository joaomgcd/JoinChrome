...
var ChromeNotification = function (notificationFromGcm) {
	...
	this.notify = function (timeout) {
		...
		doGetBase64Images(imagesToDownload, async function (results) {
			...
			var appPage = await getNotificationPage(me);
...
chrome.notifications.onClicked.addListener(async function (id) {
	...
	var opened = await openNotificationPage(notification);
	if (!opened) {
		notification.doAction(notification.actionId);
	}
});
chrome.notifications.onClosed.addListener(async function (id, byUser) {
	return;
	if (!byUser) {
		return;
	}
	...
	notification.cancel(await getAddDismissEverywhereButton());
});
