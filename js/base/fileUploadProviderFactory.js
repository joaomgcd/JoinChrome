const FileUploadProviderFactory = {
    "create": (options) => {
        const canAllSend = options.deviceIdsToSendTo.every(deviceId => UtilsDevices.canContactViaLocalNetwork(deviceId));
        const uploaders = [];
        if (canAllSend) {
    ...
    this.provide = async () => {
        const result = deviceIdsToSendTo.map(async deviceId => {
            ...
            const result = await googleDriveManager.uploadFiles({
                ...
                notify: await back.getShowInfoNotifications()
            }, filesToUpload);
