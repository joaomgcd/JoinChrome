var UtilsVoice = {
	...
	"doVoiceCommand": async function (devices, callbackPrompt, command) {
		...
		try {
			...
			var result = await UtilsVoice.voiceRecognizer.doVoiceCommand(input);

			if (!result) {
				throw "No command";
			}
			await VoiceCommandHandlers.handlers.handle(result);
			return result;
		} catch (error) {
			throw error;
		}
	},
	"toggleContinuous": async function (getDevicesFunc, wakeUp, statusFunc, callbackPrompt, callback, callbackError) {
		...
		if (status) {
			var continuousCallback = async function (command) {
				if (!command) {
					return;
				}
				UtilsVoice.voiceRecognizer.stopSpeechRecognition();
				...
				try {
					var result = await back.UtilsVoice.doVoiceCommand(getDevicesFunc(), callbackPrompt, command);
					if (!result) {
						return;
					}
					if (callback) {
