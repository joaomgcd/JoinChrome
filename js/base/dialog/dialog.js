...
Dialog.setResult = async function (result) {
	await back.dispatch("dialogresult", { result: result });
	if (window) {
		window.close();
	}
}
Dialog.cancel = async function (result) {
	if (window.dialogResultSet) {
		return;
	}
	await back.dispatch("dialogcancel", { result: result });
	if (window) {
		window.close();
	}
}
...
Dialog.resizeCurrentDialog = async function () {
	const tab = await back.getCurrentTabPromise();
	if (!tab) return;

	console.log("dialog height: " + window.document.body.offsetHeight)
	console.log(tab);
