var UtilsDevices = {
	...
	"setCanContactViaLocalNetwork": function (device, value) {
		...
		if (typeof deviceId != "string") {
			...
			device = UtilsDevices.getDevices().first(device => device.deviceId == deviceId);
		}
		const key = `localNetwork${deviceId}`;
	...
	"isHidden": async function (device) {
		return await back.getOptionValue("checkbox", device.deviceId + "disable");
	},
