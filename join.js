...
var showNotificationsPopup = function(tab){
	...
	chrome.windows.create({"focused":false, url: 'devices.html?tab='+tab+'&closeOnEmpty=true', type: 'panel' , left: screen.width - width, top: Math.round((screen.height / 2) - (height /2)), width : width, height: height},function(win){
...
var createPushClipboardWindow = function(tab,params,paramsIfClosed,closeAfterCommand){
	...
	if(popupWindowClipboard){
		...
		chrome.windows.create({ url: url, type: 'panel' , left: screen.width - 230, top: Math.round((screen.height / 2) - (height /2)), width : width, height: height},function(clipboardWindow){
...
var sendSmsFromButtonCommand = function(deviceId){
	...
	chrome.windows.create({ url: url, type: 'panel' , left: screen.width - 230, top: Math.round((screen.height / 2) - (height /2)), width : width, height: height},function(win){
		smsWindow = win;
		smsWindowId = win.id;
	});*/
